...
const cardDetail = {
  async render() {
    return `
      ...
      <div class="py-4 mx-4">
          <h3 id="pregnancy" class="text-center">Toddler needs</h3>
  ...
  async afterRender() {
    const balitaResponse = await GrowfieSrc.getProductsByCategory('Balita');

    const balitaProducts = balitaResponse.data.products;

    const articles = [...balitaProducts];
