...
const cardDetail = {
  async render() {
    return `
      ...
      <div class="py-4 mx-4">
          <h3 id="pregnancy" class="text-center">Kids needs</h3>
  ...
  async afterRender() {
    const AnakanakResponse = await GrowfieSrc.getProductsByCategory('Anak-anak');

    const AnakanakProducts = AnakanakResponse.data.products;

    const articles = [...AnakanakProducts];
